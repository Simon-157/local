...
char *ip1; // First IP address
char *ip2; // Second IP address
...
void packet_handler(u_char *user_data, const struct pcap_pkthdr *pkthdr, const u_char *packet) {
    ...
    if (ntohs(eth_header->ether_type) == ETHERTYPE_IP) {
        ...
        // Check if the packet is between the specified IP addresses
        if (strcmp(inet_ntoa(ip_header->ip_src), ip1) != 0 && strcmp(inet_ntoa(ip_header->ip_src), ip2) != 0)
            return;
        if (strcmp(inet_ntoa(ip_header->ip_dst), ip1) != 0 && strcmp(inet_ntoa(ip_header->ip_dst), ip2) != 0)
            return;
